...
class Updater(telegram.ext.Updater):
    def __init__(self,
        ...
        super().__init__(
            token=token, base_url=base_url, workers=workers, bot=bot,
            ...
            persistence=persistence, defaults=defaults, use_context=True,
            dispatcher=dispatcher, base_file_url=base_file_url)
    ...
    def command(self, filters=None, command=None):
        def _decorator(decorated):
            self.dispatcher.add_handler(telegram.ext.CommandHandler(
                ...
                decorated, filters=filters or None
            ))
